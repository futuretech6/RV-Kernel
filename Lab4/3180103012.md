...
8. L0的那个PTE的值会变成他的地址（即`*pte_addr == pte_addr`，非常神奇），而且怎么修改都没用。<u>这个错误极其恶心，是因为free list的limit设得太大了（从256MB改到16MB即可）。</u>我也不知道为什么会这样。
    ...
    * 对于一次16MB的分配，共需4096个物理页，因此要有8个三级页表，因此二级页表中需要有8个valid的PTE；高位映射和等值映射共需要2个二级页表，16个三级页表；UART映射和等值映射共享一二级页表，独占一个三级页表；总共需要1+2+17=20个页表，占空间80K
...
11. la只能32位，要64位得用li
12. paging init很慢，所以要在init完再打开中断处理
